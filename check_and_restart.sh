...
if ! pgrep -f "$PROCESS_NAME" >/dev/null; then
    ...
    nohup nice -n 10 "$HOME/pybot-env/bin/python3" -u "$HOME/bingx5_46/bingx5_46_1futures_limit.py" --air --demo > "$HOME/bingx5_46/bot_output.log" 2>&1 &
    ...
    if pgrep -f "$PROCESS_NAME" >/dev/null; then
        ...
        tail -n 25 "$HOME/bingx5_46/bot_output.log"
    fi
else
