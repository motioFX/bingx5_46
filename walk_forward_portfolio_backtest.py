...
def load_all_symbols_data(csv_path: Path) -> dict:
    ...
    for sym in symbols:
        ...
        open_s = get_col(df_sym, "open", "bybit_open").astype(float)
        high_s = get_col(df_sym, "high", "bybit_high").astype(float)
        low_s = get_col(df_sym, "low", "bybit_low").astype(float)
        close_s = get_col(df_sym, "close", "bybit_close").astype(float)
        vol_s = get_col(df_sym, "volume", "bybit_volume").astype(float)
        ...
        if "openInterest" in df_sym.columns:
            df_out["openInterest"] = df_sym["openInterest"].astype(float)
            df_out["bybit_openInterest"] = df_sym["openInterest"].astype(float)
        elif "bybit_openInterest" in df_sym.columns:
            df_out["openInterest"] = df_sym["bybit_openInterest"].astype(float)
            df_out["bybit_openInterest"] = df_sym["bybit_openInterest"].astype(float)

        if "fundingRate" in df_sym.columns:
            df_out["fundingRate"] = df_sym["fundingRate"].astype(float)
            df_out["bybit_fundingRate"] = df_sym["fundingRate"].astype(float)
        elif "bybit_fundingRate" in df_sym.columns:
            df_out["fundingRate"] = df_sym["bybit_fundingRate"].astype(float)
